...
8. **OpenAI API call (`gpt-4o-mini`)** — `generate_response()` streams the reply token-by-token into the UI via `st.empty().markdown()`.
9. **Pedagogical output check** — once the full response is assembled, `check_pedagogical_output()` (a separate `gpt-4o-mini` call, not the judge model) asks whether the response gives away a complete answer without scaffolding; if so, the response is regenerated once with an explicit corrective instruction appended to the query.
...
13. **Background evaluation thread launched** — a daemon `threading.Thread` running `run_evaluation_pipeline()` is started (non-blocking, never joined). Inside that thread: a fresh SQLite connection is opened; if a reference answer exists for the topic (via a case-insensitive **substring** match against `REFERENCE_ANSWERS`, a different, looser matching strategy than the exact-match `REFERENCE_BY_TOPIC` lookup used in step 11), ROUGE-L and BERTScore F1 are computed and logged; the user's `total_interactions` is read fresh from the database, and if it is a multiple of `JUDGE_SAMPLE_RATE` (3), the judge model (`gpt-4o`) is invoked to score the response on four dimensions, and the result (including the ROUGE-L/BERTScore F1 values) is persisted to the `evaluations` table. All exceptions inside this thread are caught, logged, and swallowed — a failure here can never crash or visibly affect the chat UI.
14. **Bookkeeping** — `bump_interactions()`, XP award via `award_xp()`, and (every `ASSESSMENT_INTERVAL` interactions) a ZPD re-assessment via `should_assess()` / `run_assessment()`. The result branches three ways: a `SCAFFOLD` recommendation silently sets `st.session_state.scaffold_mode = True` (no banner — it only changes prompt injection, not the numeric level); an `INCREASE`/`DECREASE` recommendation is shown as a pending "Adaptive Suggestion" banner **only if** that direction is not currently in a decline cooldown (`in_cooldown()`, see 3.5); `MAINTAIN`, or a recommendation still in cooldown, does nothing. `run_assessment()` also applies a down-only confusion override (`detect_confusion()`) on the chat path that can force `DECREASE` regardless of the quiz-accuracy math, but can never force `INCREASE`.

Two distinct quality-control mechanisms exist side by side and must not be conflated: the **inline regeneration gates** (steps 9–11, using `gpt-4o-mini`, live, blocking, decide whether to re-generate) and the **background evaluation pipeline** (step 13, using `gpt-4o`, sampled, non-blocking, purely for persisted measurement — it never triggers regeneration or changes what the user sees).
...
| LLM generator | OpenAI API, `gpt-4o-mini` (via the official `openai` Python SDK's `OpenAI` client) |
| LLM judge | OpenAI API, `gpt-4o` |
...
| Secrets management | `python-dotenv` (`>=1.2.2`), loading `OPENAI_API_KEY` from a `.env` file |
...
The system previously ran on Google's Gemini API (`gemini-2.5-flash`/`gemini-2.5-pro` via the `google-genai` SDK), and `requirements.txt` at that time listed the legacy `google-generativeai` package while the code actually imported the newer `google-genai` SDK — a declared-dependency/actual-import mismatch worth noting for reproducibility of that earlier version. Both the dependency declarations and the code were subsequently migrated together to the `openai` package, so no such mismatch exists for the LLM SDK in the current implementation.

A `torchvision` dependency (`>=0.28.0`) was later added to `pyproject.toml` only (not to `requirements.txt`, reintroducing a smaller version of the same kind of declared-dependency inconsistency described above). No file in the ALPS codebase imports `torchvision` directly — it exists purely to quiet `ModuleNotFoundError` noise that Streamlit's file-watcher generates when it introspects `transformers`' optional vision-model preprocessors (`transformers.models.*.image_processing_*`, none of which ALPS uses); it has no functional effect on ROUGE, perplexity, or BERTScore computation.
...
| `last_decline_direction` | TEXT, nullable | `"INCREASE"` or `"DECREASE"` — the direction of the most recently declined adaptive suggestion, set by `set_decline()` when the user clicks "Stay at current level" |
| `last_decline_interaction` | INTEGER, nullable | `total_interactions` at the moment of that decline; `in_cooldown()` compares the current `total_interactions` against this value to decide whether the same direction may resurface (see 3.5) |
...
| `conversation_id` | INTEGER, nullable, FK → conversations.id | The exact assistant row this evaluation scored, returned by `add_message()`'s `cur.lastrowid` and threaded through the eval thread's args into `save_evaluation()`. Present on every database (declared directly in `schema.sql`'s `CREATE TABLE`, and back-filled via `ALTER TABLE ... ADD COLUMN` for any database created before it existed) |
| `user_id`, `session_id` | TEXT, FKs | — |
| `judge_model` | TEXT | Always `gpt-4o` currently |
...
`rouge_l`, `bertscore_f1`, `disagreement`, `user_id`, `session_id`, and `conversation_id` on the `evaluations` table; `xp` on `users`; `was_socratic_mode` / `query_classification` on `conversations`; and `last_decline_direction` / `last_decline_interaction` on `users` (see above) were all added after the tables' original definitions — `database/db.py`'s `init_db()` runs a list of `ALTER TABLE ... ADD COLUMN` statements, each individually wrapped in a `try/except sqlite3.OperationalError`, so that a database created before these columns existed is migrated in place without error on every app start. `schema.sql`'s own `CREATE TABLE` statements have since been updated to declare `conversation_id`, `last_decline_direction`, and `last_decline_interaction` directly, so only databases created before those changes actually rely on the `ALTER TABLE` migration path for them; the `evaluations.conversation_id` column, in particular, was originally `NOT NULL` on the very first version of the schema, then temporarily dropped from `schema.sql` during the Gemini→OpenAI migration (causing insert failures on long-lived databases that still carried the old `NOT NULL` constraint), and was subsequently restored — this history is a concrete illustration of why schema drift between `schema.sql` and a long-lived `alps.db` file is worth testing for explicitly, not just assumed away by the migration mechanism.
...
- **Ollama → Gemini API → OpenAI API.** The system originally ran a fine-tuned `llama3.2:3b` model locally via Ollama (see `Modelfile.txt`, still present in the repository but now vestigial and unused by the running application). It was first migrated to Google's Gemini API for speed and reliability ahead of the paper/demo deadline, then migrated a second time to OpenAI's API (`gpt-4o-mini` for tutoring, `gpt-4o` for evaluation). `Modelfile.txt` is dead weight in the repo — it defines a `SYSTEM` prompt and generation parameters (`temperature 0.7`, `num_ctx 2048`, `num_predict 2000`) that are no longer read by any code path.
- **Single LLM judge → generator/judge model separation.** All tutoring generation, the pedagogical-scaffolding check, the ZPD slow-path recommendation, and quiz generation route through `llm/engine.py`'s four interface functions, each of which calls `client.chat.completions.create(model=OPENAI_MODEL, ...)` (`gpt-4o-mini`) directly. The bias-evaluation judge (`evaluation/judge.py`) and the style normalizer (`evaluation/style_normalizer.py`) each instantiate their own separate `OpenAI` client and call `model=JUDGE_MODEL` (`gpt-4o`) exclusively. This separation exists specifically to avoid self-enhancement bias — a model asked to judge its own output tends to score itself more favorably.
...
- **Three-way ZPD recommendation → four-way, plus a down-only confusion override and a decline cooldown.** The original ZPD state machine issued only `INCREASE` / `DECREASE` / `MAINTAIN`. It was extended with a fourth `SCAFFOLD` recommendation for the 50–65% accuracy band (see 3.5), a heuristic `detect_confusion()` check that can force `DECREASE` from any other recommendation on the chat path regardless of quiz accuracy (never the reverse), and a per-direction decline cooldown (`in_cooldown()` / `set_decline()`) that suppresses re-showing an `INCREASE`/`DECREASE` banner the learner already dismissed for `DECLINE_COOLDOWN_INTERACTIONS` (20) interactions. None of this changes the underlying accuracy thresholds in `config.ZPD`; it changes what the system does with the recommendation once computed.
...
File: `llm/engine.py`. A single OpenAI client (`OpenAI(api_key=OPENAI_API_KEY)`) is created at module import time and reused for all four interfaces; there is no shared `_call()` wrapper — each interface calls `client.chat.completions.create()` directly.

- **Interface 1 — `generate_response(query, level, socratic_mode=False, scaffold_mode=False, strictness=3, topic="General", subject_area="...", chat_history=None, stream=False)`.** Selects template level 5 if `socratic_mode=True`, else `max(1, min(level, 4))` (clamped). Loads and fills the template, builds the anchored query (see 3.3), assembles `messages = [system] + chat_history + [anchored user message]` directly in OpenAI's native `{"role": ..., "content": ...}` format — no role remapping is needed, since the database already stores `"user"`/`"assistant"` roles that match OpenAI's schema exactly (unlike the prior Gemini integration, which had to remap `"assistant"` → `"model"`). Calls `client.chat.completions.create(model=OPENAI_MODEL, messages=messages, stream=stream, temperature=0.7, max_tokens=2000)`. `chat_history` in the live app is always the last 20 messages of the **current session only** (`get_session_history(sid, n=20)`), so history never bleeds across topics. When `stream=True` (live chat display), the function wraps the returned stream in a generator that yields each non-`None` `chunk.choices[0].delta.content`; when `stream=False` (used for the three regeneration passes), it returns `response.choices[0].message.content` directly.
- **Interface 2 — `assess_level(chat_history, quiz_scores, current_level)`.** Fast path fires when `len(quiz_scores) >= 3` — this threshold is a **hardcoded literal `3` inside `engine.py`**, not read from `config.MIN_QUIZZES_FOR_ZPD` (that config constant is never imported into this file; it happens to also equal 3 in the current config, but changing `MIN_QUIZZES_FOR_ZPD` in `config.py` would have no effect on this function's behavior). Fast-path math: `< 50%` → DECREASE, `50–65%` → **SCAFFOLD**, `65–85%` → MAINTAIN, `85–95%` → INCREASE, `> 95%` → INCREASE — the 50–65% band was changed from `MAINTAIN` to a distinct `SCAFFOLD` recommendation (see 3.5) so the system can inject extra scaffolding instructions into the tutor prompt without moving the learner's numeric level. Slow path (fewer than 3 quiz scores) sends the last-10-messages conversation history to `gpt-4o-mini` with an explicit instruction set — including the same `SCAFFOLD` option for a "struggling but engaged, roughly 50–65%" case — and `response_format={"type": "json_object"}`, asking for `{"recommendation", "reasoning", "confidence"}` as JSON where `recommendation` is one of `INCREASE`/`DECREASE`/`SCAFFOLD`/`MAINTAIN`; on any parse failure it falls back to `{"recommendation": "MAINTAIN", "reasoning": <error string>, "confidence": 0.4}`.
- **Interface 3 — `generate_quiz(topic, level, num_questions=5)`.** Maps level to a difficulty description string internally (`1`→"very basic, conceptual...", `2`→"intermediate...", `3`→"advanced, involving edge cases and complexity analysis", `4`→"expert-level..."), prompts for exactly `num_questions` MCQs, and calls the API with `response_format={"type": "json_object"}`. Because OpenAI's JSON-object mode requires a top-level JSON *object* rather than a bare array (a constraint the prior Gemini integration did not have), the prompt now asks for `{"questions": [...]}` rather than a raw `[...]` array, and the code unwraps `data["questions"]` before validating each question (4 options each, 0-based `correct_answer` index, 1–2 sentence `explanation`): all four required keys present, exactly 4 options, all options non-empty strings, `correct_answer` an int in `[0, 3]`. Any question failing validation is silently dropped from the returned list; on any exception the function returns `[]`.
- **Interface 4 — `check_pedagogical_output(response_text)`.** Asks the model (`gpt-4o-mini`, `response_format={"type": "json_object"}`) two yes/no questions in one call — does the response give away a complete answer without requiring the learner to think, and does it include at least one scaffolding element — and expects `{"has_scaffolding": bool, "gives_away_answer": bool}`. On any exception it returns the safe default `(True, False)`, guaranteeing a parse failure never triggers an unwanted regeneration. If the actual result is `gives_away_answer=True AND has_scaffolding=False`, the calling code (`page_chat()`) regenerates the response once with an explicit "add scaffolding, don't give the complete answer" instruction appended to the original query.
...
- `run_assessment(user_id, current_level, check_confusion=True)` reads the **last 10 messages across all sessions for the user** (`get_history`, cross-session, not session-scoped) and the last 5 quiz scores, calls `assess_level()`, optionally applies the confusion override (below), and returns `None` if the final recommendation is `MAINTAIN` (no banner shown in that case) or the result dict otherwise (which may now be `SCAFFOLD`, not just `INCREASE`/`DECREASE`). The `check_confusion` flag defaults to `True` for the chat-path caller in `page_chat()`; the quiz-path caller in `page_quiz()` (after a non-diagnostic quiz submission) explicitly passes `check_confusion=False`, so a quiz's post-submission ZPD read is never overridden by confusion phrases from unrelated chat messages.
- **Confusion override (`detect_confusion(chat_history, window=6)`)** — a purely heuristic, non-LLM check: it lowercases the last 6 user messages and counts how many contain any of a fixed `CONFUSION_PHRASES` list (`"i don't understand"`, `"i'm confused"`, `"this doesn't make sense"`, `"i'm lost"`, `"still confused"`, `"not clear"`, `"i don't get it"`, `"can you explain again"`, `"huh?"`, and spelling variants without apostrophes). Two or more hits within that window trigger the override. Inside `run_assessment()`, if `check_confusion` is `True` and the natural recommendation is not already `DECREASE`, a detected confusion signal replaces it with a synthetic `{"recommendation": "DECREASE", "reasoning": "Confusion signals detected in recent messages (down-only override).", "confidence": 0.7}` result. This is explicitly **down-only**: confusion can force a drop from `INCREASE`, `SCAFFOLD`, or `MAINTAIN` down to `DECREASE`, but detecting the *absence* of confusion phrases never pushes a recommendation up.
- **SCAFFOLD auto-activation (no confirmation banner)** — unlike `INCREASE`/`DECREASE`, a `SCAFFOLD` recommendation is never shown as a Yes/No banner, because it does not change the numeric level. Both call sites that invoke `run_assessment()`/read a quiz's ZPD result (`page_chat()`'s bookkeeping step and `page_quiz()`'s post-submission result display) check for `recommendation == "SCAFFOLD"` first and, if so, silently set `st.session_state.scaffold_mode = True`; any other recommendation (including a fresh `MAINTAIN`) sets it back to `False`, so scaffold mode tracks only the *most recent* assessment rather than persisting once triggered. When active, `generate_response()` in `llm/engine.py` appends a fixed instruction to the system prompt — "The learner's recent quiz accuracy sits in the 50-65% band ... stay at the current level, but add extra scaffolding: break ideas into smaller steps, check understanding more often ... favor guiding questions over full explanations" — on top of whatever level-1–4 (or level-5-Socratic) template was already selected. Scaffold mode and Socratic mode are independent flags on `generate_response()` (`socratic_mode`, `scaffold_mode`) and can be active simultaneously: a Socratic-mode response gets the scaffold instruction appended to the level-5 template rather than a level 1–4 template. A small "Scaffold" badge is rendered in the sidebar (`_scaffold_badge()`, mirroring the existing `_socratic_badge()`) whenever `scaffold_mode` is active, giving the learner the same visibility Socratic mode already had.
- **Decline cooldown (`in_cooldown()` / `set_decline()`)** — direction-aware and stored per user in the `users` table (`last_decline_direction`, `last_decline_interaction`; see 2.3). When the learner clicks "Stay at current level" on either the chat-page banner or the quiz-page inline banner, `set_decline(user_id, recommendation, total_interactions)` records that direction and the interaction count at the time of decline. Before showing a new `INCREASE`/`DECREASE` banner, both call sites check `in_cooldown(user, recommendation)`, which returns `True` only if the *same* direction was declined and fewer than `DECLINE_COOLDOWN_INTERACTIONS` (20) interactions have elapsed since; `SCAFFOLD` and `MAINTAIN` are never subject to cooldown (`in_cooldown()` returns `False` immediately for anything other than `INCREASE`/`DECREASE`). A suppressed-by-cooldown recommendation is treated the same as `MAINTAIN` for banner purposes — nothing is shown, and `scaffold_mode` is turned off if it was previously on.
- Level changes are never automatic. A non-cooldown `INCREASE`/`DECREASE` result is stored in `st.session_state.assessment_pending` and rendered as an "Adaptive Suggestion" banner directly below the latest chat message (i.e., at the bottom of the visible scroll area, not the page top) with "Yes, update level" / "Stay at current level" buttons. Only clicking "Yes" calls `apply_recommendation()`, which clamps the new level to `[1, 4]` (`min(current+1, 4)` for INCREASE, `max(current-1, 1)` for DECREASE) and writes it to the database; `apply_recommendation()` is a no-op (returns the current level unchanged) for `SCAFFOLD` or `MAINTAIN`, though in practice the app never calls it with those values since they never reach the Yes/No banner.
...
The normal (non-diagnostic) quiz setup screen lets the user pick a topic, a question count via a slider (3–10), and a difficulty level via a selectbox (defaulting to the user's current level). Topic selection is no longer purely free-text: `get_studied_topics(user_id)` in `database/db.py` joins `conversations` to `sessions` and returns the distinct topics the learner has actually chatted about, most-recently-active first; if that list is non-empty, the setup screen shows a selectbox populated with those topics plus a trailing `"Type your own..."` option (which reveals a free-text input when selected). Only when the learner has no chat history yet — `get_studied_topics()` returns an empty list — does the screen fall back to the original plain `st.text_input()`. This means quiz topics are now generally drawn from what the learner has actually studied, while still allowing an arbitrary topic to be typed. The diagnostic-mode override (see 3.1) bypasses all of this: it fixes `num_questions = 3` and derives both topic and level from `current_level` automatically, with no slider, dropdown, or button — generation starts as soon as the diagnostic-mode setup branch is reached.
...
Already described functionally in 3.4 (Interface 4). To restate its integration precisely: `check_pedagogical_output()` runs **on every assistant response, unconditionally**, immediately after the streamed response is fully assembled and before any of the other two regeneration gates. It uses a single `gpt-4o-mini` call (the generator model, **not** the judge model — this is a same-model self-check, distinct from the separately-modeled LLM-as-Judge pipeline described in Section 4). It acts as a synchronous, blocking safety net that is fully independent of whether Socratic mode is active — even a Level 4, non-Socratic response can be caught and forced to add scaffolding if it gives away a complete answer.
...
**Judge metrics** (every 3rd response, `JUDGE_SAMPLE_RATE = 3`):
...
File: `evaluation/reference_answers.py`. Nine topics are covered, each with one fixed expert-written reference answer and a `level` metadata field (which does **not** gate anything — the same single reference text is used for ROUGE/BERTScore comparison regardless of the learner's actual `current_level`):
...
The bank was extended from an original five topics (all pitched at Level 1–2) to nine, adding `arrays` (Level 1) and three Level 3/4-pitched topics — `binary trees`, `graphs`, and `dynamic programming` — so the reference bank now spans all four proficiency levels rather than topping out at "level 2."

This still has an important, non-obvious consequence for interpreting the ROUGE-L/BERTScore-per-level table, but it is now a **two-directional** artifact rather than the earlier one-directional "no L3/L4 coverage" gap. For the six topics whose reference is pitched at Level 1 or 2 (`arrays`, `linked lists`, `binary search`, `recursion`, `sorting algorithms`, `hash tables`), a Level 4 learner's response is still compared, via ROUGE/BERTScore, against that same lower-pitched reference text — a correct, more advanced response (more technical vocabulary, different structure, deeper complexity discussion) will structurally tend to show **lower** lexical/semantic overlap with it than a Level 1/2 response would, purely because the reference itself is not level-matched, not because the higher-level response is worse. Conversely, for `binary trees` and `graphs` (Level 3) and `dynamic programming` (Level 4), the same artifact now runs the other way for *lower*-level learners: a genuinely correct Level 1 explanation of "graphs" will be compared against a Level-3-pitched reference and will tend to show lower overlap purely because it is simpler, not because it is wrong. Any observed "gap" in mean ROUGE-L/BERTScore F1 across levels should therefore still be interpreted as a possible measurement artifact of the fixed, non-level-adjusted reference bank — now a bidirectional effect rather than a one-directional coverage gap — and this caveat should be stated explicitly wherever the ROUGE/BERTScore-per-level table is discussed in the paper (this is a different, narrower caveat than the `language_neutrality` judge dimension, which does not depend on a reference answer at all and is not subject to this artifact).

Two distinct topic-matching strategies exist for this same reference bank: the live in-chat regeneration gate (`page_chat()`, Gate 3) uses an **exact-match** lookup (`REFERENCE_BY_TOPIC.get(topic.lower())`), while the background evaluation pipeline's `find_reference()` uses a **substring match** (any reference topic string contained within the session's topic string, case-insensitively) — meaning a session topic like "Binary Search Trees" would fail the exact-match gate (no regeneration triggered against a reference) but succeed the substring match in the background pipeline (a ROUGE-L/BERTScore F1 value would still be computed and persisted for it). Topics with no matching reference at all (e.g., "Tries", "Greedy Algorithms", or any free-typed topic genuinely outside the nine listed) get `NULL` for `rouge_l` and `bertscore_f1` in every persisted evaluation row; `dump_metrics.py`'s Section 4 reports the exact count of such `NULL` rows.
...
Decision: `gpt-4o-mini` generates tutoring content; `gpt-4o` (a separate, more capable model) performs all bias/quality judging. Cite: the CALM framework's characterization of self-enhancement bias — a model scoring its own generations tends to inflate those scores relative to an independent evaluator.
...
### 5.7 Sampling the judge at 1-in-3 responses
Decision: `JUDGE_SAMPLE_RATE = 3`. Rationale: under the prior Gemini free tier, hard rate limits (as low as 25 requests/day for `gemini-2.5-pro` on some tiers, and per-minute request caps) made it infeasible to judge every single tutor response, so sampling was originally set to 1-in-5. After migrating to OpenAI's paid API tier, denser sampling became affordable, and the rate was tightened to 1-in-3. Framing for the paper: this should be presented as a deliberate, literature-consistent sampling strategy (evaluation sampling is common practice in the LLM-evaluation literature), not apologized for as a shortfall.
...
### 5.9 A fourth ZPD state, a down-only confusion override, and a decline cooldown
Decision: the 50–65% accuracy band was split out of `MAINTAIN` into its own `SCAFFOLD` recommendation that changes prompt behaviour (more scaffolding, smaller steps) without moving the learner's numeric level or asking for confirmation; a heuristic phrase-match confusion detector was added that can force a level `DECREASE` but never an `INCREASE`; and repeated `INCREASE`/`DECREASE` suggestions the learner has already declined are suppressed for a cooldown window rather than re-shown on every subsequent assessment. Rationale: a learner scoring 50–65% is meaningfully different from one scoring 65–85% — genuinely struggling but not yet failing badly enough to justify dropping a level — and treating both identically as `MAINTAIN` discarded a real signal the ZPD fast path already had available for free. The confusion override adds a second, faster-reacting downward safety net alongside the slower quiz-driven assessment, deliberately asymmetric (down-only) so a false-positive confusion match can never inappropriately push a learner up. The decline cooldown addresses a UX failure mode where a learner who has explicitly said "no" to a suggestion would otherwise see the identical suggestion again at the very next assessment interval if their quiz/conversation signal had not changed.
...
### 6.3 Rate limiting under the prior Gemini free tier (resolved by the OpenAI migration)
The Gemini API free tier previously constrained both the judge sample rate (5.7) and, more broadly, how much live testing/evaluation data could be collected. This was addressed by migrating to OpenAI's paid API tier, which removed the free-tier request caps and allowed the judge sample rate to be tightened from 1-in-5 to 1-in-3 (5.7). The remaining constraint is now direct per-call dollar cost rather than a hard rate limit — a materially different, more manageable limitation, but still worth disclosing as a factor bounding how much evaluation data could be collected.
...
### 6.6 Stuck-detection is heuristic and interval-gated, not continuous
A `detect_confusion()` heuristic now exists (see 3.5) and can force a `DECREASE` recommendation independently of quiz accuracy, partially closing what was previously a complete gap. However, two limitations remain, and the paper should not overstate what this mechanism does: (1) it is a fixed-phrase substring match (`"i'm confused"`, `"i don't understand"`, etc.), not a semantic or LLM-based confusion classifier, so a learner expressing confusion in different words will not trigger it; (2) it is only evaluated when `should_assess()` fires — i.e., every `ASSESSMENT_INTERVAL` (10) interactions on the chat path, or after a quiz submission with `check_confusion=False` (so never on the quiz path in practice) — not continuously on every turn. A learner can therefore still send several confused messages in a row without the system reacting until the next assessment boundary, or a low quiz score is reached. Ideal solution: a continuous, semantically-aware confusion-signal classifier running on every turn rather than a fixed keyword list checked at fixed intervals.
...
`normalize_style()` is a full additional OpenAI call before every judge score, roughly doubling the judge-side API cost/latency per sampled evaluation. This is mitigated by running the entire evaluation pipeline in a non-blocking background thread, so it does not add latency to the user-facing chat experience, but it does add real cost/quota pressure (compounding with 6.3).
...
- Be precise: say "gpt-4o-mini" or "gpt-4o", not "a large language model."
